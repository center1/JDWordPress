...
if ( file_exists( WP_CONTENT_DIR . '/jae_config.php' ) ){
...
elseif ( file_exists( dirname( WP_CONTENT_DIR ) . '/important-sample.php' ) )
	$config_file = file( dirname( WP_CONTENT_DIR ) . '/important-sample.php' );
else
	wp_die( __( 'Sorry, I need a important-sample.php file to work from. Please re-upload this file from your WordPress installation.' ) );

// Check if wp-config.php has been created
if ( file_exists( WP_CONTENT_DIR.'/important.php' ) && !file_exists(WP_CONTENT_DIR.'/reinstall.lock'))
	wp_die( '<p>' . sprintf( __( "The file 'important.php' already exists. If you need to reset any of the configuration items in this file, please delete it first. You may try <a href='%s'>installing now</a>." ), 'install.php' ) . '</p>' );

// Check if wp-config.php exists above the root directory but is not part of another install
if ( file_exists(ABSPATH . '../wp-config.php' ) && !file_exists(WP_CONTENT_DIR.'/reinstall.lock') && ! file_exists( ABSPATH . '../wp-settings.php' ) )
...
if(isset($jae_dbhost) && !empty($jae_dbhost) && $step <2){
	$step=2;
}
...
<?php
		...
		if ( file_exists( WP_CONTENT_DIR.'/important-sample.php' ) )
			...
			$path_to_wp_config = dirname( WP_CONTENT_DIR ) . '/important.php';
